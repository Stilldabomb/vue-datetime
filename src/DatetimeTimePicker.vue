<template>
  <div :class="{'vdatetime-time-picker': true, 'vdatetime-time-picker__with-suffix': use12Hour, 'vdatetime-time-picker__with-seconds': useSeconds}">
    <div class="vdatetime-time-picker__list vdatetime-time-picker__list--hours" ref="hourList">
      <div class="vdatetime-time-picker__item" v-for="hour in hours" @click="selectHour(hour)"
           :class="{'vdatetime-time-picker__item--selected': hour.selected, 'vdatetime-time-picker__item--disabled': hour.disabled}">{{ formatHour(hour.number) }}
      </div>
    </div>
    <div class="vdatetime-time-picker__list vdatetime-time-picker__list--minutes" ref="minuteList">
      <div class="vdatetime-time-picker__item" v-for="minute in minutes" @click="selectMinute(minute)"
           :class="{'vdatetime-time-picker__item--selected': minute.selected, 'vdatetime-time-picker__item--disabled': minute.disabled}">{{ minute.number }}
      </div>
    </div>
    <div class="vdatetime-time-picker__list vdatetime-time-picker__list--seconds" ref="secondList" v-if="useSeconds">
      <div class="vdatetime-time-picker__item" v-for="second in seconds" @click="selectSecond(second)"
           :class="{'vdatetime-time-picker__item--selected': second.selected, 'vdatetime-time-picker__item--disabled': second.disabled}">{{ second.number }}
      </div>
    </div>
...
<script>
  import { hours, minutes, pad, seconds, timeComponentIsDisabled } from './util'
...
<style>
  ...
  .vdatetime-time-picker__with-seconds .vdatetime-time-picker__list {
    width: 33.3%;
  }

  .vdatetime-time-picker__with-suffix.vdatetime-time-picker__with-seconds .vdatetime-time-picker__list {
    width: 25%;
  }
